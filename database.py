...
def get_pool():
    ...
    if _pool is None:
        _pool = psycopg2.pool.ThreadedConnectionPool(
            minconn=1,
            maxconn=15,  # Supabase free tier giới hạn ~15 connections
            dsn=DATABASE_URL,
            sslmode='require'
        )
    return _pool
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS word_errors (
        id SERIAL PRIMARY KEY,
        user_id INTEGER NOT NULL,
    ...
    for uname, pw, role in [('admin','123456','admin'), ('user','123456','user')]:
        existing = fetchone(conn, "SELECT id FROM users WHERE username=%s", (uname,))
        if not existing:
    ...
    #     if not fetchone(conn, "SELECT id FROM topics WHERE name=%s AND owner_id IS NULL", (name,)):
    #         execute(conn, "INSERT INTO topics (name,description,owner_id) VALUES (%s,%s,NULL)", (name, desc))
    # conn.commit()

    # t = {r['name']: r['id'] for r in fetchall(conn, "SELECT * FROM topics WHERE owner_id IS NULL")}
    ...
    # ]
    # for v in vdata:
    #     if not fetchone(conn, "SELECT id FROM vocabulary WHERE hanzi=%s AND topic_id=%s AND owner_id IS NULL", (v[1], v[0])):
    #         execute(conn, """INSERT INTO vocabulary
    #             (topic_id,hanzi,pinyin,vietnamese,example_sentence,example_pinyin,example_vietnamese,owner_id)
    #             VALUES (%s,%s,%s,%s,%s,%s,%s,NULL)""", v)
    ...
    # ]
    # for s in sdata:
    #     if not fetchone(conn, "SELECT id FROM sentences WHERE hanzi=%s AND topic_id=%s AND owner_id IS NULL", (s[1], s[0])):
    #         execute(conn, "INSERT INTO sentences (topic_id,hanzi,pinyin,vietnamese,owner_id) VALUES (%s,%s,%s,%s,NULL)", s)
